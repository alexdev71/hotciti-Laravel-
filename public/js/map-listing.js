$(document).ready(function(){
    ...
    $.ajax({  
       ...
       cache: false,
        success: function(data) {
            console.log(data);
                    ...
                    for (var i in data) { 
                       ...
                       locationURL.push(data[i].City);
                       locationImg.push(data[i].Picture_Links);
                       locationTitle.push(data[i].City);
                       locationAddress.push(data[i].Zip_Code);
                       locationCategory.push(data[i].Region);
                       locationStarRating.push(data[i].Income_Taxes);
                       locationRevievsCounter.push(data[i].Income_Taxes);
                       locationStatus.push(data[i].City);
                     ...
                     (function ($) {
                      ...
                      function locationData(locationURL, locationImg, locationTitle, locationAddress, locationCategory, locationStarRating, locationRevievsCounter, locationStatus,lat,long,marker,image) {
        ...
        for(var k=0, l = marker.length; k < l; k++){
          ...
          loca.push([locationURL[k], parseFloat(lat[k]), parseFloat(lon[k]), parseFloat(marker[k]), image[k]],);
    ...
    markerImg = loca[i][4];
    markerCount = loca[i][3] + 1;
        var overlaypositions = new google.maps.LatLng(loca[i][1], loca[i][2]),
        ...
        return function() {
             ...
             boxText.innerHTML = loca[i][0];
             ib.close();
             ib.open(map, overlay);
             currentInfobox = loca[i][3];
 
                    var latLng = new google.maps.LatLng(loca[i][1], loca[i][2]);
                    map.panTo(latLng);
                    map.panBy(0, -110);
